...
class WolfPet {
public:
  enum Action { ACT_FEED = 0, ACT_PLAY, ACT_REST, ACT_COUNT };

  void begin();                  // load persisted state
  void tick(unsigned long now);  // decay over time + autosave (call every loop)
  void doAction(int action);     // ACT_FEED / ACT_PLAY / ACT_REST
