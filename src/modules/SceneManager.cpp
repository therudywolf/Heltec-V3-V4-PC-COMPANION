...
// ===========================================================================
// ASSETS: CYBER-WOLF DAEMON (32x32 XBM)
// Verified pixel-art for Idle and Aggressive states.
// ===========================================================================

// IDLE STATE (Calm, observing)
static const unsigned char wolf_idle_bits[] = {
    ...
    0xE0, 0xFF, 0xFF, 0x03, 0xF0, 0xFF, 0xFF, 0x07, 0xF8, 0xEF, 0xF7, 0x0F,
    0xFC, 0xC7, 0xE3, 0x1F, 0xFC, 0x83, 0xC1, 0x1F, 0xFE, 0x01, 0x80, 0x3F,
    0xFE, 0x01, 0x80, 0x3F, 0xFE, 0x00, 0x00, 0x3F, 0xFC, 0x10, 0x04, 0x1F,
    0xFC, 0x38, 0x0E, 0x1F, 0xF8, 0x38, 0x0E, 0x0F, 0xF0, 0x7C, 0x1F, 0x07,
    0xE0, 0xFF, 0xFF, 0x03, 0xC0, 0xFF, 0xFF, 0x01, 0x00, 0xFF, 0x7F, 0x00,
    0x00, 0xFE, 0x3F, 0x00, 0x00, 0xFC, 0x1F, 0x00, 0x00, 0xF0, 0x07, 0x00,
    0x00, 0xC0, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// ALERT STATE (High CPU/RAM, bared teeth)
static const unsigned char wolf_alert_bits[] = {
    };
...
// Displays found networks with RSSI bars and security status.
// Allows scrolling and interaction.
// ===========================================================================
void SceneManager::drawWiFiScanner(int selectedIndex, int pageOffset) {
  ...
  if (n == -2) {
    disp_.drawCentered(32, "INITIALIZING SCAN...");
    WiFi.scanNetworks(true);
    return;
  } else if (n == -1) {
    disp_.drawCentered(32, "SCANNING AIRWAVES...");
    // Simple loading bar animated by time
    int w = (millis() / 50) % 100;
    u8g2.drawFrame(14, 40, 100, 6);
    u8g2.drawBox(16, 42, w, 2);
    return;
  } else if (n == 0) {
    disp_.drawCentered(32, "NO TARGETS FOUND");
    disp_.drawCentered(45, "[LONG PRESS TO RESCAN]");
    return;
  }
  ...
  u8g2.setFont(LABEL_FONT);
  char headerMsg[32];
  snprintf(headerMsg, sizeof(headerMsg), "TARGETS: %d [%d-%d]", n,
           pageOffset + 1, min(pageOffset + 5, n));
  disp_.drawTechFrame(0, 0, 128, 12);
  u8g2.drawUTF8(4, 9, headerMsg);

  // --- List Items (Max 5 per page) ---
  int yStart = 20;
  int h = 9;
  u8g2.setFont(TINY_FONT);
  ...
  for (int i = pageOffset; i < min(pageOffset + 5, n); i++) {
    ...
    // 1. SSID (Truncate if too long)
    String ssid = WiFi.SSID(i);
    if (ssid.length() > 14)
      ssid = ssid.substring(0, 13) + ".";
    u8g2.drawUTF8(4, y, ssid.c_str());

    // 2. Security Lock Icon
    wifi_auth_mode_t enc = WiFi.encryptionType(i);
    bool isOpen = (enc == WIFI_AUTH_OPEN);
    u8g2.drawUTF8(90, y, isOpen ? "OPEN" : "LOCK");

    // 3. RSSI Bar Graph (Signal strength)
    int rssi = WiFi.RSSI(i); // Typically -90 to -40
    int bars = map(constrain(rssi, -90, -50), -90, -50, 1, 4);
    for (int b = 0; b < bars; b++) {
      u8g2.drawBox(115 + (b * 3), y - 2 - (b * 1), 2, 2 + (b * 1));
    }
  ...
  if (pageOffset > 0)
    u8g2.drawTriangle(120, 14, 124, 14, 122, 11); // Up arrow
  if (pageOffset + 5 < n)
    u8g2.drawTriangle(120, 62, 124, 62, 122, 59); // Down arrow
}
