...
3. Открой `.env` в блокноте. Основные параметры:
   ...
   | `PC_IP`    | IP твоего ПК в сети               | Нужен только для справки; в плату вписывается в `main.cpp` (раздел CONFIGURATION) |
...
1. Открой **main.cpp** и в начале файла (раздел CONFIGURATION) задай WiFi и адрес ПК:
...
2. Для прошивки нужны: **main.cpp**, **platformio.ini** (или настройки платы в Arduino IDE).
3. Открой проект в **PlatformIO** (файл `platformio.ini` в корне), выбери плату `heltec_wifi_lora_32_V3` (или совместимую ESP32 с SSD1306), собери и залей прошивку. Либо открой **main.cpp** в Arduino IDE, выбери плату Heltec WiFi Kit 32, собери и залей.
...
- Проверь, что `PC_IP` в **main.cpp** совпадает с реальным IP ПК.
