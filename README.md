...
В текущем билде (v2.0) мини-игра Wolf & Moon не входит — 11 экранов.

**При старте:** экран приветствия Forest OS → By RudyWolf с волчьей анимацией.

**Алерты:** пороги CPU/GPU по температуре и нагрузке настраиваются в `main.cpp` (CPU_TEMP_ALERT, GPU_TEMP_ALERT и др.). При превышении — мигающая рамка и надпись ALERT, LED (если включён в меню).
...
**Не коммитить в Git:** `.env`, папки сборки и exe.
...
1. Открой **main.cpp** и в начале файла (раздел CONFIGURATION) задай **WIFI_SSID**, **WIFI_PASS**, **PC_IP** (IP ПК, например `192.168.1.2`), **TCP_PORT** (8888). Опционально: **CPU_TEMP_ALERT**, **GPU_TEMP_ALERT**, **CPU_LOAD_ALERT**, **GPU_LOAD_ALERT** для порогов алертов (дефолты заданы через `#ifndef` ниже).
2. Собери и залей прошивку в Heltec: открой проект в **PlatformIO** (файл `platformio.ini` в корне), выбери плату `heltec_wifi_lora_32_V3` (или совместимую), собери и залей. Либо используй Arduino IDE с установленными платами ESP32 и библиотеками U8g2, ArduinoJson.
3. Плата подключится к WiFi и к TCP-серверу на ПК — на дисплее появятся данные.

### 4. Управление на плате

- **Короткое нажатие** — следующий экран (11 экранов по кругу).
- **Долгое нажатие** — открыть меню. В меню: короткое — следующий пункт (LED → AUTO → Bright → Intvl → EQ → EXIT), долгое на пункте — вкл/выкл или смена значения (контраст 128/192/255, интервал 5/10/15 с, стиль эквалайзера) / выход.

Настройки LED, карусели, контраста, интервала и стиля эквалайзера сохраняются в NVS.

### 5. Устранение неполадок

- **Нет данных на дисплее:** проверь, что LibreHardwareMonitor запущен и веб-сервер включён; открой в браузере `http://localhost:8085/data.json` и убедись, что в JSON есть нужные SensorId (как в `TARGETS` в `monitor.py`). Проверь WiFi и **PC_IP** в **main.cpp**, порт 8888 и брандмауэр.
- **Не показывается обложка трека:** нужен PIL (`pip install Pillow`); обложка приходит в каждом пакете при её наличии (экран Player — screen 6). Убедись, что воспроизведение идёт через приложение с поддержкой Windows Media API.
